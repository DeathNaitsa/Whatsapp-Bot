...
async function getOrCreateUser(message, senderName) {
    ...
    const isOwner = userIds.includes(config.bot.owner);
    ...
    if (user) {
        ...
        // Auto-assign owner rank if this is the owner
        if (isOwner && (!user.teamm || user.teamm !== 'ersteller')) {
            user.teamm = 'ersteller';
            user.registered = true;
            user.dsgvoAccepted = true;
            needsUpdate = true;
            console.log(`👑 Owner erkannt - Ersteller-Rang zugewiesen: ${primaryId}`);
        }
        ...
        user = {
            ...
            platformIds: userIds,
            registered: isOwner ? true : false,
            registeredAt: isOwner ? Date.now() : null,
            dsgvoAccepted: isOwner ? true : false,
            teamm: isOwner ? 'ersteller' : undefined,
            level: 0,
            exp: 0,
            money: 0,
            coins: isOwner ? 1000 : 0,
            items: {},
            cooldown: {},
            commandsUsed: 0,
            messagesCount: 0,
            lastSeen: Date.now(),
            _temporary: isOwner ? false : true
        };
        
        if (isOwner) {
            console.log(`👑 Owner erstmalig erkannt - Account erstellt: ${primaryId}`);
        }
    }
...
async function main() {
    ...
    // Use session ID from config or default to 'bot'
    const sessionId = config.bot.sessionId || 'bot';
    const sessions = [sessionId];
    
    console.log(`📱 Starte Session: ${sessionId}\n`);
    
    for (const sessionId of sessions) {
        const sessionPath = path.join(__dirname, 'sessions', sessionId);
        
        try {
            await startSession(sessionId, {sql:true});
